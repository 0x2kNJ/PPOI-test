...
// The security level to apply in the hash_to_field op
const SECURITY_LEVEL = 128

// RFC domain separation tag
const DST_SK_SPEND = 'bermuda:sk_spend:v1'
...
function bigint2bytes(b: bigint, out: Uint8Array = new Uint8Array(32)): Uint8Array {
...
function bytes2bigint(buf: Uint8Array): bigint {
  return buf.reduce(
    (acc, byte) => (acc << 8n) | (BigInt(byte) & 255n),
    0n
  )
}

/** I2OSP(RFC 8017): non-negative integer to fixed-length big-endian bytes. */
function I2OSP(value: number | bigint, length: number): Uint8Array {
  const v = BigInt(value)
  if (v < 0n) throw new Error('I2OSP: negative integer')
...
function OS2IP(os: Uint8Array): bigint {
  return bytes2bigint(os)
}

/** Concat given byte arrays. */
function concatBytes(...subarrays: Uint8Array[]): Uint8Array {
...
function strxor(a: Uint8Array, b: Uint8Array): Uint8Array {
  if (a.length !== b.length) throw new Error('strxor: length mismatch')
...
function expandMessageXMD(
  ...
  if (ell > 255) throw new Error('expandMessageXMD: ell > 255')
  ...
  const blocks: Uint8Array[] = new Array(ell)
...
function hashToField(
  ...
  count: number,
  dst: string,
  p: bigint = BN254_FIELD_SIZE,
  k: number = SECURITY_LEVEL
): bigint[] {
...
export function hashToFieldScalar(
  msg: Uint8Array | string,
  dst: string = DST_SK_SPEND,
  p: bigint = BN254_FIELD_SIZE,
  k: number = SECURITY_LEVEL
): bigint {
